...
export default function Sidebar({ hidden = false }) {
  ...
  if (!hidden || (channel && category)) {
    return (
      <div className="w-[18%] pb-20 dark:bg-stone-900 h-screen overflow-y-scroll scrollbar-hide hidden lg:flex flex-col pr-2">
        {/* <ItemMenu /> */}
        {/* <div className='p-[0.2px] h-[1px] w-[90%] px-2 mx-auto dark:bg-stone-700 bg-gray-100'></div> */}
        ...
        <div className="mt-2 px-2">
          {category?.map((res, index) => (
            <Link
              key={index}
              href={`/category/${res?.id}/${res?.category_title.toLowerCase()}`}
            >
              <div
                key={index}
